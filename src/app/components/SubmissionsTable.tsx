...
export function SubmissionsTable({ data, onExport }: SubmissionsTableProps) {
  ...
  const handleExport = (format: "pdf" | "excel") => {
    const headers = ["Student", "Project", "Uniqueness Score", "Confidence", "Status", "Submission Date", "Review note"];
      ...
      <div className="px-4 py-3 border-b border-border flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input value={search} onChange={e => { setSearch(e.target.value); setPage(0); }}
            ...
            aria-label="Search submissions by student, project, or HU ID"
            autoComplete="off"
            spellCheck={false}
        ...
        <select value={scoreFilter} onChange={e => { setScoreFilter(e.target.value); setPage(0); }}
          className="px-3 py-2 rounded-lg border border-border text-sm bg-white">
          <option value="all">All uniqueness scores</option>
          ...
          <thead>
            <tr className="border-b border-border bg-gray-50/50">
              ...
              <th className="px-4 py-3 text-left">
                <span style={{ fontSize: 11, fontWeight: 600, color: "#64748B", textTransform: "uppercase" }}>Review note</span>
              </th>
            </tr>
          </thead>
          <tbody>
            {paged.map((s, i) => {
              ...
              const sc = statusColors[s.status] || statusColors.pending || { bg: '#F1F5F9', text: '#475569' };
              return (
                <motion.tr key={s.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}
                  ...
                  <td className="px-4 py-3">
                    <span className="px-2 py-0.5 rounded-full text-xs font-semibold capitalize"
                      style={{ background: sc.bg, color: sc.text }}>{stageLabel(s.stage)}</span>
                  </td>
