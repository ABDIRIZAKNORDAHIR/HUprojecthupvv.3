...
import { LayoutDashboard, ClipboardList, FolderKanban, Users, Settings, MessageSquare } from "lucide-react";
import { roleAccent } from "../config/brandTheme";
...
const studentItems: NavItem[] = [
  ...
  { id: "class-assignments", icon: ClipboardList, label: "Tasks" },
  { id: "settings", icon: Settings, label: "Settings" },
];

const teacherItems: NavItem[] = [
  { id: "dashboard", icon: LayoutDashboard, label: "Home" },
  { id: "ai-queue", icon: ClipboardList, label: "Review", badgeKey: "pendingReview" },
  { id: "students", icon: Users, label: "Students" },
  { id: "class-assignments", icon: ClipboardList, label: "Assign" },
  { id: "messages", icon: MessageSquare, label: "Messages" },
];
        ...
        {navItems.map(item => {
          ...
          return (
            <motion.button
              ...
              style={{ background: isActive ? `color-mix(in srgb, ${accent} 11%, transparent)` : 'transparent' }}
            >
              <item.icon size={20} style={{ color: isActive ? accent : "#64748B" }} />
              <span style={{ fontSize: 10, fontWeight: isActive ? 700 : 500, color: isActive ? accent : "#64748B" }}>
                {item.label}
              </span>
              {badge && badge > 0 && (
