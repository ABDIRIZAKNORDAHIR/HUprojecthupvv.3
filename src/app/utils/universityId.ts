...
export function validateUniversityId(raw: string): { ok: true; id: string } | { ok: false; error: string } {
  ...
  if (digits.length % 4 !== 0) {
    return { ok: false, error: 'Digits must be in groups of 4 (4, 8, 12, or 16 digits)' };
  }
  return { ok: true, id };
}

export const UNIVERSITY_ID_HINT = 'HU000 then digits in groups of 4 (e.g. HU000-1234 or HU000-1234-5678)';
